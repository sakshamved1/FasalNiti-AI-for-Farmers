/**
 * Master Dataset of all 36 Indian States and Union Territories with official districts
 * Used for standardized location selection across farmers, buyers, mandis, and schemes
 */
...
const getMasterStates = () => INDIAN_STATES_AND_DISTRICTS.map(s => s.name);

const getDistrictsByState = (stateName) => {
  if (!stateName) return [];
  const stateObj = INDIAN_STATES_AND_DISTRICTS.find(s => 
    s.name.toLowerCase() === stateName.trim().toLowerCase() ||
    s.id === stateName.trim().toLowerCase().replace(/\s+/g, '-')
  );
